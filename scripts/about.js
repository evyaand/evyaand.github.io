...
function storyMarkup(entry, compact = false) {
  ...
  const galleryControls =
    images.length > 1
      ? `
        <button
          class="about-gallery-arrow about-gallery-prev"
          type="button"
        ...
        <button
          class="about-gallery-arrow about-gallery-next"
          type="button"
  ...
  return `
    ...
    >
      ...
      .map(
        (paragraph) =>
          `<p class="about-index-text">${paragraph}</p>`
      )
      .join("")}
      </div>
    </article>
...
function renderNodes(container) {
  ...
  function scrollToStoryOnMobile() {
    if (
      !window.matchMedia(
        "(max-width: 650px)"
      ).matches
  ...
  function stopDrag(event) {
    ...
    if (
      graph.hasPointerCapture(
        event.pointerId
      )
    ) {
      graph.releasePointerCapture(
        event.pointerId
      );
    }
...
function renderOrigins(container) {
  ...
  container.innerHTML = `
    ...
    >
      ...
      <div class="origins-data-layout">
        <div
          class="origins-list"
          role="list"
          aria-label="Ethnicity estimate regions"
        >
          ${ORIGIN_REGIONS.map(
    (region) => `
              <button
                type="button"
                data-origin-id="${region.id}"
                role="listitem"
                aria-pressed="false"
              >
                <span
                  class="origins-swatch"
    ...
    <section
      class="origins-field is-unselected"
      aria-label="Regional map of the ethnicity estimate"
    >
      <div
        class="origins-field-grid"
        aria-hidden="true"
      ></div>
    ...
    (region) => `
            <span
              class="origins-zone origins-zone-${region.id}"
              data-origin-zone="${region.id}"
              style="--origin-color:${region.color}"
            ></span>
          `
  ).join("")}
      </div>
